...
import { ReservationSubmitButton } from "@/app/components/Buttons/ReservationSubmitButton";
import { CategoryShowcase } from "@/app/components/CategoryShowcase";
import { HomeMap } from "@/app/components/Map/HomeMap";
...
export default async function HomeRoute({
  ...
  const county = getCountryByValue(data?.country as string);
  ...
  return (
    <div className="w-[75%] mx-auto mt-10 mb-20">
      ...
      <div className="relative h-[400px]">
        <Image
          src={`https://szqooclehvyztrwtqxsy.supabase.co/storage/v1/object/public/images/${data?.photo}`}
          alt="photo"
          fill
      ...
      <div className="flex justify-between gap-x-24 mt-8">
        <div className="w-2/3">
          ...
          <div className="flex gap-x-2 text-muted-foreground">
            <p className="">Guests {data?.guests}</p> *{" "}
            <p className="">Bedrooms {data?.bedroom}</p> *{" "}
            <p className="">Bathrooms {data?.bathroom}</p>
          </div>
          ...
          <HomeMap locationValue={county?.value as string} />
        </div>
